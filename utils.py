...
def calculate_bbx_loop_range(data_dict, calc_config=CalculationConfig(), eval_config=StaticEvaluationConstants()):
    dt = data_dict['dt']
    total_lines = data_dict["total_lines"]
    step_size = 1

    # Find the start index based on time_start
    start_idx = next(i for i, t in enumerate(data_dict[header_time]) if t >= eval_config.time_start)

    # Handle cases where time_stop exceeds the available range
    if eval_config.time_stop > data_dict[header_time][-1]:
        stop_idx = total_lines - 1  # Include the entire remaining range
    else:
        stop_idx = next(i for i, t in enumerate(data_dict[header_time]) if t > eval_config.time_stop) - 1
